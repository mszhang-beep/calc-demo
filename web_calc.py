...
if page == "🧮 计算器":
    ...
    if st.session_state.curve_x is not None:
        ...
        ax2.plot(st.session_state.curve_x, st.session_state.curve_y1, color="#ff69b4", linewidth=2, label=f"y = {func1}")
        if st.session_state.curve_y2 is not None:
        ...
        if slide_valid and y_slide is not None:
            ...
            ax2.plot(x_slide, y_slide, "orange", marker="o", markersize=7, zorder=10, label=f"当前点({x_slide:.2f},{y_slide:.2f})")

        ax2.axhline(y=0, color="gray", linewidth=0.8, linestyle="--")
        ax2.axvline(x=0, color="gray", linewidth=0.8, linestyle="--")
        ax2.set_xlabel("x")
        ax2.set_ylabel("y")
        ax2.set_title("函数图像｜橙色标记为滑块当前位置")
        ax2.legend()
